...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	TODOPatch_float = TODOPatch * 1.0
	textureIm_float = textureIm * 1.0
	for r in range(ssd_rows):
		for c in range(ssd_cols):
			...
			# center (x, y) of window in textureIm
			row_center_textureIm = c + patchL
			col_center_textureIm = r + patchL
			# compute SSD. First we create the square window with dimensions (2*patchL + 1, 2*patchL + 1, 3)
			# we set the dimensions by only taking the tectureIm_float from ranges center-patchL to center+patchL + 1
			window_float = textureIm_float[col_center_textureIm-patchL : col_center_textureIm+patchL + 1, row_center_textureIm-patchL : row_center_textureIm + patchL + 1, :]
			# compute per channel
			square_difference = np.square(TODOPatch_float - window_float)
			# sum
			SSD[r, c] = np.sum(square_difference[TODOMask==0])
			pass
		pass
	return SSD
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	'''
	Copies the selected patch into the final image imHole
	'''
	...
	for i in range(patchSize):
		for j in range(patchSize):
			...
			row_hole = iPatchCenter - patchL + i
			col_hole = jPatchCenter - patchL + j
			row_mask = i
			col_mask = j
			row_texture = iMatchCenter - patchL + i
			col_texture = jMatchCenter - patchL + j
			
			if TODOMask[row_mask, col_mask]:
				imHole[row_hole, col_hole, :] = textureIm[row_texture, col_texture, :]
			pass
		pass
	return imHole
...
im = Image.open('a3_6_before_1_sd_10_patchL_10.jpg').convert('RGB')
